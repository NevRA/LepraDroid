...
public interface CommentsUpdateListener extends UpdateListener
{
    ...
    void OnCommentsUpdateFirstEntries(UUID groupId, UUID postId, int count, int totalCount, int commentToSelect);
    void OnCommentsUpdate(UUID groupId, UUID postId, int count);
    void OnCommentsUpdateFinished(UUID groupId, UUID postId, int commentToSelect);
}
